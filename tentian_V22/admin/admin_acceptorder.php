...
$sql = "SELECT order_id, order_name, full_name, date_created, quantity, price, status FROM orders WHERE status = 'pending'";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['accept'])) {
        ...
        if (!empty($remarks)) {
            ...
            $updateSql = "UPDATE orders SET status = 'Approved' WHERE order_id = :order_id";
        ...
        if (!empty($remarks)) {
            ...
            $updateSql = "UPDATE orders SET status = 'Declined' WHERE order_id = :order_id";
            $updateStmt = $conn->prepare($updateSql);
            $updateStmt->bindValue(':order_id', $orderId);
            $updateStmt->execute();
